...
if True:
    ...
    f = Form(False)
    #Define File array
    filea = open("client.ip")
    fxa = str(filea.read()).split('\n')
    fxb = {}
    x=0
    ...
    filea.close()
    print "FXA ",fxa
    fxba = [x for x in fxb if x != 0]
    print "FXB ",fxb
    # Add objects

    sel = Select(border_width=5, top=50)
    f.add_object('text', Text('Raspberry Pi Login - Select Pi', label_style=['bold','underline']))
    for z in fxb:
        print z
        print "len ",len(fxb)
        if z != len(fxb)-1:
            sel.add_option(fxb[z][2], z+1)
            print fxb[z][2],":",z+1

    f.add_object('select', sel)
    f.add_object('code', Input('Code: ', 'default', label_size=18, input_border_color=(255,100,100), input_border_width=3, input_bg_color=(0,0,0,0)))
    ...
    for kv in r.iteritems():
        ...
        if kv[0] == "select":
            ...
            for n in range(0,len(fxa)-1):
                if int(kv[1]) == int(fxb[n][1]):
                    codeCorrect = True
    pygame.quit()
    if codeCorrect:
        os.system("python client.py "+ IP)
    # End
